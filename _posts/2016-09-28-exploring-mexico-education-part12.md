...
subtitle: Part12 - Basic mapping in R
...
The two R packages that we will be using are: <br>maptools <br>ggplot2
...
According to our shapefiles, there are 31 states and 1 federal district in Mexico. However our dataset have 33 unique entries. In order to map, we need to get our dataset to be compatible with the shapefiles.
...
``` r
map = map + theme_nothing()
plot(map)
```

![](mxhtml1_files/figure-markdown_github/unnamed-chunk-14-1.png)
